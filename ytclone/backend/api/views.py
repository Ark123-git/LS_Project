...
#     if user in creator.subscribers.all():
#         creator.subscribers.remove(user)
#         return Response({'subscribed': False, 'message': 'Unsubscribed'})
#     else:
#         creator.subscribers.add(user)
#         return Response({'subscribed': True, 'message': 'Subscribed'})


# @api_view(['GET'])
# @permission_classes([permissions.IsAuthenticated])
# def is_subscribed(request, video_id):
#     video = get_object_or_404(Video, id=video_id)
#     creator = video.user
#     user = request.user

#     return Response({'subscribed': user in creator.subscribers.all()})
...
#     if creator in user.subscribed_to.all():
#         user.subscribed_to.remove(creator)
#         return Response({'subscribed': False, 'message': 'Unsubscribed'})
#     else:
#         user.subscribed_to.add(creator)
#         return Response({'subscribed': True, 'message': 'Subscribed'})
...
#     return Response({'subscribed': creator in user.subscribed_to.all()})
...
# Add subscription field if not already present
if not hasattr(User, 'subscribed_to'):
    User.add_to_class('subscribed_to', models.ManyToManyField('self', symmetrical=False, related_name='subscribers'))
